    ...
    def handle(self, *args, **options):
        ...
        for course_name in courses_data:
            course, created = Course.objects.get_or_create(
                ...
                defaults={
            'course_description': f'Описание курса "{course_name}"'
                }
            )
            if created:
                created_courses.append(course)
                self.stdout.write(
            self.style.SUCCESS(f'Создан курс: {course.course_name}')
                )
            else:
                created_courses.append(course)
                self.stdout.write(
            self.style.WARNING(f'Курс "{course_name}" уже существует')
                )
        ...
        for i, (lesson_name, lesson_desc) in enumerate(lessons_data):
            ...
            if created:
                ...
                self.stdout.write(
            self.style.SUCCESS(f'Создан урок: {lesson.lesson_name}')
                )
            else:
                created_lessons.append(lesson)
                self.stdout.write(
            self.style.WARNING(f'Урок "{lesson_name}" уже существует')
                )
